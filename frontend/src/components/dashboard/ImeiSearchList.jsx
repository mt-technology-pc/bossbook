import { useRef, useState } from 'react'
import { ScanLine, CheckSquare, Square, AlertCircle } from 'lucide-react'

// Shared search-and-tick UI for picking serial/IMEI units from a known
// list: a live-filtering search box plus a checkbox list, with Enter
// auto-selecting on a full exact match (a barcode scanner sends the whole
// code at once) or, once typing has narrowed the list to exactly one
// remaining match, on that partial match too. Purely presentational over
// whatever `units` it's given — callers own fetching/filtering the list
// itself (ImeiPicker fetches its own; SaleImeiModal is handed an
// already-computed available-units list instead).
export default function ImeiSearchList({ units, value = [], onChange, requiredCount, notFoundLabel = 'unit' }) {
  const [scanInput, setScanInput] = useState('')
  const [scanError, setScanError] = useState('')
  const inputRef = useRef(null)

  const toggle = (unitId) => {
    setScanError('')
    onChange(value.includes(unitId) ? value.filter((id) => id !== unitId) : [...value, unitId])
  }

  const query = scanInput.trim().toLowerCase()
  const filteredUnits = query
    ? units.filter((u) => u.serial_number.toLowerCase().includes(query))
    : units

  const handleScan = (e) => {
    if (e.key !== 'Enter' && e.key !== 'Tab') return
    e.preventDefault()
    const serial = scanInput.trim()
    if (!serial) return

    let match = units.find((u) => u.serial_number === serial)
    if (!match) {
      const selectable = filteredUnits.filter((u) => !value.includes(u.id))
      if (selectable.length === 1) match = selectable[0]
    }

    if (!match) {
      setScanError(
        filteredUnits.length > 1
          ? `Multiple matches for "${serial}" — keep typing or tap the one you want.`
          : `"${serial}" not found among available ${notFoundLabel}s`,
      )
      return
    }
    if (value.includes(match.id)) {
      setScanError(`"${match.serial_number}" already selected`)
      return
    }
    onChange([...value, match.id])
    setScanInput('')
    setScanError('')
    inputRef.current?.focus()
  }

  const countLabel = requiredCount ? `${value.length} / ${requiredCount} selected` : `${value.length} selected`
  const countOk = !requiredCount || value.length === requiredCount

  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="flex items-center gap-1.5 text-xs font-medium text-ink-500">
          <ScanLine size={12} /> IMEI / Serial
        </span>
        <span className={`text-xs font-semibold ${countOk ? 'text-green-600' : 'text-amber-600'}`}>
          {countLabel}
        </span>
      </div>

      <div className="mb-2 flex gap-1.5">
        <input
          ref={inputRef}
          value={scanInput}
          onChange={(e) => { setScanInput(e.target.value); setScanError('') }}
          onKeyDown={handleScan}
          placeholder="Scan or search IMEI, press Enter"
          className="flex-1 rounded-lg border border-ink-400/20 bg-cream-50 px-2.5 py-1.5 text-xs text-ink-900 placeholder:text-ink-400 outline-none focus:border-clay-500 focus:ring-2 focus:ring-clay-500/20"
        />
      </div>

      {scanError && (
        <p className="mb-2 flex items-center gap-1 text-xs text-red-600">
          <AlertCircle size={11} /> {scanError}
        </p>
      )}

      {units.length === 0 ? (
        <p className="py-2 text-center text-xs text-ink-400">No {notFoundLabel}s available</p>
      ) : filteredUnits.length === 0 ? (
        <p className="py-2 text-center text-xs text-ink-400">No match for &quot;{scanInput.trim()}&quot;</p>
      ) : (
        <div className="max-h-56 space-y-0.5 overflow-y-auto">
          {filteredUnits.map((u) => {
            const selected = value.includes(u.id)
            return (
              <button
                key={u.id}
                type="button"
                onClick={() => toggle(u.id)}
                className={`flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs transition-colors ${
                  selected
                    ? 'bg-clay-500/10 text-clay-700'
                    : 'text-ink-700 hover:bg-cream-200'
                }`}
              >
                {selected
                  ? <CheckSquare size={13} className="shrink-0 text-clay-600" />
                  : <Square size={13} className="shrink-0 text-ink-300" />}
                <span className="font-mono">{u.serial_number}</span>
              </button>
            )
          })}
        </div>
      )}
    </div>
  )
}
