  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border border-ink-400/15 bg-cream-50 py-16 text-center dark:border-cream-100/10 dark:bg-dark-800">
        ...
        <>
          <div className="mt-6 max-w-lg rounded-2xl border border-ink-400/15 bg-cream-50 p-6 dark:border-cream-100/10 dark:bg-dark-800">
            <div className="flex items-center justify-between py-2">
              <span className="text-sm font-medium text-ink-600 dark:text-cream-300">Revenue</span>
              <span className="font-heading text-lg font-semibold text-ink-900 dark:text-cream-50">
                {formatCurrency(totalRevenue)}
              </span>
            </div>
            <div className="flex items-center justify-between border-t border-ink-400/10 py-2 dark:border-cream-100/10">
              <span className="text-sm font-medium text-ink-600 dark:text-cream-300">Cost of Goods Sold</span>
              <span className="font-heading text-lg font-semibold text-ink-900 dark:text-cream-50">
                ({formatCurrency(totalCogs)})
              </span>
            </div>
            <div className="flex items-center justify-between border-t-2 border-ink-400/20 py-3 dark:border-cream-100/20">
              <span className="text-sm font-bold text-ink-900 dark:text-cream-50">Gross Profit</span>
              <span className="font-heading text-xl font-bold text-ink-900 dark:text-cream-50">
                {formatCurrency(grossProfit)}
              </span>
            </div>
            <div className="flex items-center justify-between pb-2">
              <span className="text-xs text-ink-400">Gross margin</span>
              <span className="text-xs font-medium text-ink-500 dark:text-cream-400">{formatPercent(grossMargin)}</span>
            </div>

            <div className="flex items-center justify-between border-t border-ink-400/10 py-2 dark:border-cream-100/10">
              <span className="text-sm font-medium text-ink-600 dark:text-cream-300">Operating Expenses</span>
              <span className="font-heading text-lg font-semibold text-ink-900 dark:text-cream-50">
            ...
            <div className="flex items-center justify-between border-t-2 border-clay-500/40 py-3">
              <span className="text-sm font-bold text-ink-900 dark:text-cream-50">Net Income</span>
              <span className={`font-heading text-2xl font-bold ${netIncome >= 0 ? 'text-clay-600 dark:text-clay-400' : 'text-red-500'}`}>
                {formatCurrency(netIncome)}
              </span>
            </div>
            <div className="flex items-center justify-between pt-1">
              <span className="text-xs text-ink-400">Net margin</span>
              <span className="text-xs font-medium text-ink-500 dark:text-cream-400">{formatPercent(netMargin)}</span>
            </div>
          </div>

          <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-ink-400/15 bg-cream-50 p-5 dark:border-cream-100/10 dark:bg-dark-800 sm:p-6">
                ...
                <table className="w-full min-w-[420px] text-left text-sm">
                  ...
                  <tbody>
                    {categories.map((c, i) => (
                      ...
                      >
                        <td className="py-2.5 pr-3 font-medium text-ink-900 dark:text-cream-50">{c.category}</td>
                        <td className="py-2.5 pr-3 text-right text-ink-700 dark:text-cream-200">{formatCurrency(c.revenue)}</td>
                        <td className="py-2.5 pr-3 text-right text-ink-500 dark:text-cream-400">{formatCurrency(c.cogs)}</td>
                        <td className="py-2.5 text-right font-semibold text-ink-900 dark:text-cream-50">{formatCurrency(c.grossProfit)}</td>
                      </motion.tr>
                    ))}
                    {categories.length === 0 && (
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-ink-400/20 text-sm font-bold dark:border-cream-100/20">
                      ...
                      <td className="pt-3 pr-3 text-right text-ink-900 dark:text-cream-50">{formatCurrency(totalRevenue)}</td>
                      <td className="pt-3 pr-3 text-right text-ink-900 dark:text-cream-50">{formatCurrency(totalCogs)}</td>
                      <td className="pt-3 text-right text-ink-900 dark:text-cream-50">{formatCurrency(grossProfit)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
              ...
              <div className="mt-4 overflow-x-auto">
                <table className="w-full min-w-[280px] text-left text-sm">
                  ...
                  <tbody>
                    {expenseCategories.map((e, i) => (
                      <motion.tr
                        ...
                        className="border-b border-ink-400/5 last:border-0 dark:border-cream-100/5"
                      >
                        <td className="py-2.5 font-medium text-ink-900 dark:text-cream-50">{e.category}</td>
                        <td className="py-2.5 text-right text-ink-700 dark:text-cream-200">{formatCurrency(e.amount)}</td>
                      </motion.tr>
                    ))}
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-ink-400/20 text-sm font-bold dark:border-cream-100/20">
                      <td className="pt-3">Total</td>
                      <td className="pt-3 text-right text-ink-900 dark:text-cream-50">{formatCurrency(totalExpenses)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
