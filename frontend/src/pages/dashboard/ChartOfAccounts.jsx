    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="mt-6 flex flex-col items-center justify-center rounded-2xl border border-dashed border-ink-400/20 bg-cream-50 py-16 text-center dark:border-cream-100/15 dark:bg-dark-800">
          <p className="text-sm font-medium text-ink-600 dark:text-cream-300">No accounts posted yet</p>
          ...
          {byType.map((group, gi) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-ink-400/15 bg-cream-50 p-5 dark:border-cream-100/10 dark:bg-dark-800 sm:p-6"
              >
                ...
                <ul className="mt-3 divide-y divide-ink-400/10 dark:divide-cream-100/10">
                  {group.accounts.map((a) => (
                    <li
                      key={a.coa_id}
                      onClick={() => navigate(`/dashboard/reports/general-ledger/${a.coa_id}`)}
                      className="flex cursor-pointer items-center justify-between gap-3 py-3 hover:bg-cream-100 dark:hover:bg-dark-700"
                    >
                      <div className="flex min-w-0 items-center gap-2">
                        <span className="truncate text-sm font-medium text-ink-900 dark:text-cream-50">
                          {a.name}
                        </span>
                        <span className="shrink-0 rounded-full bg-ink-400/10 px-2 py-0.5 text-[10px] font-semibold uppercase text-ink-400 dark:bg-cream-100/10">
                          {a.normal_balance}
                        </span>
                      </div>
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <ChevronRight size={14} className="text-ink-300 dark:text-cream-100/30" />
                      </div>
                    </li>
                  ))}
                </ul>
